...
void Window::setShadowMode(int state) {
    if (state==0) {
        WITH_SHADOWS = false;
        return;
    }
...
void Window::setLightSampling(int density) {
    LIGHT_SAMPLING = density;

    char labelText [30];
    sprintf(labelText, "Samples per Light: %d", density);

    emit lightSamplingChanged(QString(labelText));
}
...
void Window::initControlWidget () {
    ...
    QCheckBox * shadowCheckBox = new QCheckBox("Compute shadows", rayGroupBox);
    connect(shadowCheckBox, SIGNAL(stateChanged(int)), this, SLOT(setShadowMode(int)));
    rayLayout->addWidget(shadowCheckBox);
    QLabel * lightSamplingLabel = new QLabel("Samples per Light: 1", rayGroupBox);
    connect(this, SIGNAL(lightSamplingChanged(QString)), lightSamplingLabel, SLOT(setText(QString)));
    rayLayout->addWidget(lightSamplingLabel);
    QSlider * lightSamplingSlider = new QSlider(Qt::Horizontal, rayGroupBox);
    lightSamplingSlider->setTickPosition(QSlider::TicksBothSides);
    lightSamplingSlider->setRange(1, 30);
    lightSamplingSlider->setTickInterval(5);
    connect(lightSamplingSlider, SIGNAL(valueChanged(int)), this, SLOT(setLightSampling(int)));
    rayLayout->addWidget(lightSamplingSlider);
